...
class HyperbolicSolver:
    ...
    def calculate(self, N, K):
        ...
        for j in range(0, N - 1):
            ...
            if self.data.approximation == 'p1':
                ...
                u[1][j] = self.data.psi1(x) + self.data.psi2(x) * self.tau +                           (self.data.psi1_dir2(x) + self.data.b * self.data.psi1_dir1(x) +
    ...
    def implicit_solver(self):
        ...
        for k in range(2, K):
            ...
            if self.data.bound_type == 'a1p2':
                ...
                a[-1] = -self.data.gamma / (omega - self.sigma) *                         (1 + xi + 2 * self.sigma - self.data.c * self.tau ** 2) - 4 * self.data.gamma
                ...
                c[0] = -2 * self.data.a / self.h + self.h / self.tau ** 2 - self.data.c * self.h +                        -self.data.d * self.h / (2 * self.tau) +                        self.data.beta / self.data.alpha * (2 * self.data.a + self.data.b * self.h)
                d[0] = self.h / self.tau ** 2 * (u[k - 2][0] - 2 * u[k - 1][0]) - self.h * self.data.f() +                        -self.data.d * self.h / (2 * self.tau) * u[k - 2][0] +                        (2 * self.data.a - self.data.b * self.h) / self.data.alpha * self.data.phi0(k * self.tau)
                a[-1] = -b[0]
                d[-1] = self.h / self.tau ** 2 * (-u[k - 2][0] + 2 * u[k - 1][0]) + self.h * self.data.f() +                         self.data.d * self.h / (2 * self.tau) * u[k - 2][0] +                         (2 * self.data.a + self.data.b * self.h) / self.data.alpha * self.data.phi1(k * self.tau)
    ...
    def _left_bound_a2p2(self, u, k, t):
        n = self.data.c * self.h - 2 * self.data.a / self.h - self.h / self.tau ** 2 - self.data.d * self.h /             (2 * self.tau) + self.data.beta / self.data.alpha * (2 * self.data.a - self.data.b * self.h)
    ...
    def _right_bound_a2p2(self, u, k, t):
        n = -self.data.c * self.h + 2 * self.data.a / self.h + self.h / self.tau ** 2 + self.data.d * self.h /             (2 * self.tau) + self.data.delta / self.data.gamma * (2 * self.data.a + self.data.b * self.h)
    ...
    def _left_bound_a2p3(self, u, k, t):
        ...
        return self.data.alpha / denom * u[k - 1][2] - 4 * self.data.alpha / denom * u[k - 1][1] +                2 * self.h / denom * self.data.phi0(t)

    def _right_bound_a2p3(self, u, k, t):
        denom = 2 * self.h * self.data.delta + 3 * self.data.gamma
        return 4 * self.data.gamma / denom * u[k - 1][-2] - self.data.gamma / denom * u[k - 1][-3] +                2 * self.h / denom * self.data.phi1(t)
    ...
    def explicit_solver(self):
        ...
        for k in range(2, K):
            ...
            for j in range(1, N - 1):
                ...
                u[k][j] = u[k - 1][j + 1] * tmp1 +                           u[k - 1][j] * (-2 * self.sigma + 2 + self.data.c * quadr) +                           u[k - 1][j - 1] * tmp2 - u[k - 2][j] + quadr * self.data.f()
